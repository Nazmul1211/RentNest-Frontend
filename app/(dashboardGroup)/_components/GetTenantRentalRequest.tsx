...
const GetTenantRentalRequest = async ({ page = 1 }: { page?: number }) => {
    ...
    const pageSize = 6;
    const totalPages = Math.max(1, Math.ceil(requestsList.length / pageSize));
    const currentPage = Math.min(Math.max(page, 1), totalPages);
    const visibleRequests = requestsList.slice((currentPage - 1) * pageSize, currentPage * pageSize);
    ...
    return (
        <div className="space-y-4 my-4">
            ...
            <DashboardPagination currentPage={currentPage} totalPages={totalPages} totalItems={requestsList.length} pageSize={pageSize} unit="requests" />
        </div>
    );
};
